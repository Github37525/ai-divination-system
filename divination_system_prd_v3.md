# 数智易学全平台沉浸式体验需求文档（PRD v3.1）

> 文档状态：原生 App 转向版
> 更新日期：2026-08-01  
> 上一版本：[PRD v2.0](divination_system_prd_v2.md)  
> 本次主题：iOS / Android 成为下一阶段主线；H5 承接分享与验证；微信只保留文化学习版和内部研发版
> 专业与竞品基线：[六爻 / 奇门专业基准与原生 App 体验蓝图](docs/PROFESSIONAL_BENCHMARK_AND_NATIVE_APP_UX.md)
...
| 层级 | 平台 | 产品定位 | 结论 |
| --- | --- | --- | --- |
| P0 | iOS / Android 原生 App | 下一阶段主产品；极致传感器、触觉、离线和系统级体验 | SwiftUI 与 Jetpack Compose 分端实现，共享规则契约，不以 H5 套壳代替 |
| P0 | 响应式 H5 / PWA | 最快触达、独立分享、原生下载前体验 | 继续承担能力验证和链接分享，不再作为极致交互上限 |
| P1 | 桌面 Web | 深度阅读、排盘、复盘、手机协同 | 作为分析主屏，不强求电脑具备运动传感器 |
| 维护 | 微信文化学习小程序 | 《周易》原典与术语的公开学习入口 | 纯经典阅读，不接入个性化问题、随机生成、模型解释或互动 H5 跳转 |
| 内部 | 微信内部体验小程序 | 设备运动、震动与接口的白名单研发测试 | 不公开提审，不作为转发运营渠道，只用于真机验证 |
...
- 对外只宣传经过规则说明、自动化测试、独立锚点和版本记录支持的能力。
- “规则可追溯、排盘与 AI 分离、结果可审计”可以作为核心差异点；“最准、全面领先、准确率最高”不得出现。
- 起盘前和结果页必须显示当前流派、时间约定、规则版本和已知限制。
- 专业模式与新人模式必须共用同一份确定性盘面，只改变信息密度和解释方式。
- 奇门当前只支持“时家转盘奇门·拆补法”；在完成基准盘和独立评审前，不通过隐藏开关扩展其他流派。
...
补充背景问题必须在进入摇卦前完成，默认最多 1 个高价值问题且可以跳过；不得在六爻已经完成后用连续问卷阻断确定性盘面。完整盘面不以会员或模型成功作为前置条件。
...
- 提供“快速模式”和“沉浸模式”：快速模式目标 10–20 秒完成六爻；沉浸模式保留逐枚落定、触觉和声音，但不强迫观看不可跳过的长动画。
...
- 首页默认按“问一件事、继续未完成、查看历史”组织，而不是展示全部术数功能清单。
- 起卦、规则校验、模型解读和复盘必须各自具有可恢复状态；杀进程或换网后不能丢失已锁定爻。
- 原生客户端必须提供新人/专业双层视图、规则说明卡、六爻节点进度和真实的模型处理阶段。
- 模型长任务必须显示离散阶段、已用时间和可解释的预计范围；允许离开结果页继续浏览，完成后通过应用内状态和可选本地通知提醒。
- 首次完整白话结论和确定性盘面不得被付费墙分割；付费能力用于更深的批注、对比、专家工具和额外模型用量。
...
- 起盘前显示“本次方法、时间约定、AI 是否参与排盘”三项摘要。
- 结果首屏固定为“一句话看懂 / 现在可以做 / 需要留意”，随后展开确定性依据。
- 每条模型结论应关联到焦点爻、值使宫、日月关系或其他确定性字段；无法关联的内容必须标注为“模型补充视角”。
- 奇门九宫默认聚焦与问题相关的 1–3 宫；专业模式才展开完整宫位图层和术语。
- 明确显示“已支持 / 未支持”的流派与规则，不用功能营销掩盖专业边界。
...
该批次当时不包含真实物理引擎级 3D 铜钱、分层声音、账号绑定和原生 App；微信文化学习版与内部体验版作为互相隔离的独立工程推进。
...
### 当前 P0：iOS / Android 原生可信体验

- 先交付“问题输入 → 规则确认 → 体感/点按/手工 → 六爻锁定 → 确定性盘面 → 异步白话”的完整闭环。
- iOS 使用 SwiftUI + Core Motion/Core Haptics；Android 使用 Jetpack Compose + Sensor Framework/平台触觉。
- 实现新人/专业双层视图、六节点进度、真实模型阶段、后台继续、失败恢复和首个完整免费结论。
- 将当前已验证的规则、历史、反馈、跨设备会话和 DeepSeek 异步接口接入原生客户端。
- 建立六爻与奇门专业基准集；奇门达到至少 30 个独立盘例后才扩展流派或宣传专业升级。
...
- 六爻增加用神工作流、专业显示图层、卦例标签/批注/图片、前后盘对比和应验复盘。
- 奇门增加可追溯的核心格局、宫位组合、用神定位和更多公开基准盘；每项规则独立测试和标明来源。
- 完成至少两名独立长期使用者的盘面对照评审，不以模型文案替代规则核验。

### P2：原生极致版

- 完成融合运动识别、低延迟语义触觉和离线规则包。
- 完成系统分享、通知、动态字体和本地加密。
- 完成高/中/低真机帧率、功耗、热量、触觉一致性和无障碍门禁。
