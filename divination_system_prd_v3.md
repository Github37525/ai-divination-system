...
- 建立 Web/H5、PWA、微信小程序、iOS、Android、桌面浏览器的统一体验规范。
...
| P1 | 微信小程序 | 中国用户低门槛主入口 | 原生接入设备运动、震动、分享和订阅能力 |
...
| 能力 | 桌面 Web | 手机 H5 / PWA | 微信小程序 | iOS 原生 | Android 原生 |
| --- | --- | --- | --- | --- | --- |
| 摇动识别 | 无则使用键鼠或手机协同 | 条件支持；必须先探测并在需要时请求权限 | 支持设备运动监听 | Core Motion | Android Sensor Framework |
| 方向/倾斜视差 | 鼠标、触控板替代 | 条件支持 | 支持 | 支持 | 支持 |
| 触觉反馈 | 通常无，使用视觉和声音 | 能力差异大，仅特性探测后启用 | 使用小程序震动 API | UIKit / Core Haptics | HapticFeedback / VibrationEffect |
| 3D 铜钱物理动画 | 强 | 中高，按设备性能降级 | 中高，按基础库与设备降级 | 强 | 强 |
| 空间音效 | 支持，需用户主动开启 | 支持，禁止自动播放 | 支持 | 支持 | 支持 |
| 离线壳与缓存 | 可选 | PWA 支持部分离线 | 小程序包与缓存 | 完整支持 | 完整支持 |
| 推送/提醒 | 浏览器能力差异 | PWA 能力差异 | 订阅消息 | 系统通知 | 系统通知 |
| 快速分享 | 链接/二维码 | Web Share/链接 | 微信会话与卡片 | 系统分享 | 系统分享 |
| 极致体验等级 | A | B+ | A- | S | S |
...
3. 手机扫码打开 H5 或小程序，无需强制登录即可加入本次临时会话。
...
- 使用设备运动监听实现体感摇卦，使用短震动实现结果反馈。
- 支持将“无问题明文”的结果卡片分享到会话或生成海报。
- 支持订阅“事后复盘提醒”，由用户明确选择日期和是否接收。
- 小程序的运动与震动实现必须走平台能力适配器，不与 H5 代码混用。
...
- **微信小程序**：原生小程序核心页面 + 共享 TypeScript 领域 SDK；必要时使用小程序 Canvas/WebGL。
...
第一批暂不包含真实物理引擎级 3D 铜钱、分层声音、账号绑定、跨实例断线恢复、微信小程序和原生 App；这些不阻塞首批闭环验收，分别进入后续 P1 加强项、P2 和 P3。
...
- 原生接入设备运动、震动、微信分享和订阅提醒。
- 完成小程序包体、低端机和微信版本兼容测试。
- 与 H5、桌面共享规则结果和历史数据。
...
- Android：主流高端、中端、低端机；Chrome、微信小程序；有/无陀螺仪。
...
| 风险 | 影响 | 控制策略 |
| --- | --- | --- |
| H5 运动/震动能力碎片化 | 不同浏览器体验不一致 | 特性探测、权限前置说明、点按降级、小程序承接 |
...
- 微信小程序设备运动监听：[wx.startDeviceMotionListening](https://developers.weixin.qq.com/miniprogram/dev/api/device/motion/wx.startDeviceMotionListening.html)
- 微信小程序短震动：[wx.vibrateShort](https://developers.weixin.qq.com/miniprogram/dev/api/device/vibrate/wx.vibrateShort.html)

---

## 21. v3.0 完成定义

1. 五类客户端全部使用同一规则契约，同一输入的确定性结果 100% 一致。
2. 手机体感、点按降级、手工录入三条起卦路径均可独立完成六爻。
3. 电脑—手机配对、断线恢复、二维码失效和结果一致性通过端到端测试。
4. 原始运动数据不出端，权限拒绝不影响核心功能。
5. 视觉、声音、触觉均具有独立开关、能力探测和无障碍替代。
6. H5、小程序、iOS、Android 和桌面真实设备矩阵完成验收。
